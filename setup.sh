...
if [ -f /.dockerenv ] && ! hash  2>/dev/null && whoami | grep root; then
    () {
        ${*}
    }
fi
...
install_apt() {
     apt-get update || true
     apt-get install -y git gdb python3-dev python3-pip python3-setuptools libglib2.0-dev libc6-dbg

    if uname -m | grep x86_64 > /dev/null; then
         dpkg --add-architecture i386 || true
         apt-get update || true
         apt-get install -y libc6-dbg:i386 || true
    fi
}
...
if linux; then
    ...
    case $distro in
        ...
        "gentoo")
            install_emerge
            if ! hash  2>/dev/null && whoami | grep root; then
                () {
                    ${*}
                }
            fi
